...
export const w19: Week = {
  ...
  concept: [
    {
      kind: "text",
      body: "DP는 어렵게 느껴지지만, 하는 일은 **한 번 푼 문제의 답을 적어 두고 다시 안 푸는 것**뿐이에요. 계단을 오르는데 '5번째 칸까지 오는 방법'이 궁금하면, '4번째까지 오는 방법'과 '3번째까지 오는 방법'을 더하면 돼요. 그 둘도 같은 방식으로 더 작은 답에서 나와요. 작은 답부터 표에 적어 두면, 큰 답은 표를 더하기만 하면 나와요.",
    },
    {
      kind: "text",
      body: "9주차에 그리디가 지는 걸 봤어요. 후회 없는 선택을 하려면 모든 경우를 따져야 하는데, 그냥 따지면 지수 시간이에요. 동적 계획법(DP)은 **이미 계산한 하위 문제를 다시 계산하지 않음**으로써 그것을 다항 시간으로 만들어요. 쓸 수 있는 조건은 13주차 재귀에서 이미 봤어요. 중복되는 하위 문제가 있고(피보나치), 하위 문제의 최적해로 전체 최적해를 만들 수 있으면 돼요.",
    },
    {
      kind: "heading",
      body: "점화식 세우는 4단계",
    ...
    {
      kind: "heading",
      body: "직접 채워 보기: 계단 오르기",
    },
    {
      kind: "text",
      body: "한 번에 1칸이나 2칸을 오를 수 있을 때, 5번 칸에 오는 방법이 몇 가지인지 표를 채워 볼게요. `dp[i]`는 'i번 칸에 도달하는 방법 수'예요. i번 칸은 (i−1)번에서 한 칸, (i−2)번에서 두 칸 뛰어야만 닿으니까 `dp[i] = dp[i-1] + dp[i-2]`가 돼요. 가장 작은 두 칸만 손으로 채우면, 나머지는 앞의 두 칸을 더하기만 하면 나와요.",
    },
    {
      kind: "trace",
      caption: "계단 5칸 — dp 표가 왼쪽부터 채워지는 과정",
      lines: [
        "i      0   1   2   3   4   5",
        "dp     1   1   .   .   .   .     ← 초기값만 채운 상태",
        "",
    ...
    {
      kind: "text",
      body: "표를 왼쪽부터 채웠다는 게 **계산 순서**의 뜻이에요. `dp[5]`를 구할 때 `dp[4]`와 `dp[3]`이 이미 적혀 있어야 하니까요. 순서만 지키면 각 칸은 딱 한 번씩만 계산해요. 그래서 `O(n)`이에요.",
    },
  ],
  patterns: [
    {

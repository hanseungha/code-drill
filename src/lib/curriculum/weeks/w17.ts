...
export const w17: Week = {
  ...
  concept: [
    {
      kind: "text",
      body: "백트래킹은 **미로에서 갈림길을 하나씩 시도해 보는 사람**과 같아요. 갈림길에 서면 일단 한 길을 골라 들어가 보고, 막다른 곳이 나오면 마지막 갈림길로 되돌아와(백트랙) 방금 고른 길을 지우고 다음 길을 시도해요. 이렇게 하면 모든 길을 빠짐없이 훑을 수 있어요. 여기에 하나만 더 얹으면 백트래킹이에요 — 들어가기도 전에 '이 길은 어차피 막혔다'가 보이면 아예 안 들어가요. 이 '미리 안 들어가기'가 가지치기(pruning)예요.",
    },
    {
      kind: "text",
    ...
    {
      kind: "heading",
      body: "직접 따라가 보기: [1,2,3]의 순열",
    },
    {
      kind: "text",
      body: "`[1,2,3]`으로 순열 6개를 만드는 과정을 결정 트리로 따라가 볼게요. 각 단계에서 아직 안 쓴 수 중 작은 것부터 골라요. 끝까지 3개를 다 고르면 순열 하나가 완성돼요(체크). 그러고 나서 **한 칸 되돌아와(백트랙) 방금 넣은 걸 빼고** 다음 수를 시도해요. 세로줄을 따라 내려가면 선택이고, `← 백트랙`이 붙은 줄이 되돌아오는 순간이에요.",
    },
    {
      kind: "trace",
      caption: "[1,2,3] 순열 — 고르고, 완성되면 되돌아와 다음을 시도",
      lines: [
        "고른다 1",
        "  고른다 2",
        "    고른다 3  ->  [1,2,3]  완성",
    ...
    {
      kind: "text",
      body: "순열 하나가 완성될 때마다 되돌아와 방금 선택을 지우는 게 보이죠? 이 '지우기'가 코드의 `path.pop()`이고, `used[i] = false`로 그 수를 다시 쓸 수 있게 풀어주는 부분이에요. 되돌리기를 빠뜨리면 다음 가지가 이전 가지의 흔적을 그대로 물려받아 엉뚱한 답이 나와요.",
    },
    {
      kind: "heading",
      body: "가지치기가 실력이에요",

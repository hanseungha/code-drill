...
export const w02: Week = {
  ...
  concept: [
    {
      kind: "text",
      body: "1주차에서 반복문 안의 `includes`가 `O(n²)`을 만든다는 걸 봤습니다. 해시는 정확히 그 문제를 없애는 도구입니다. 값을 넣어두면 '있었나?'를 평균 `O(1)`에 답합니다.",
    },
    {
      kind: "text",
    ...
    },
    {
      ...
      body: "Python에서 `d[[1, 2]]`는 `TypeError: unhashable type`입니다. 튜플 `d[(1, 2)]`로 바꿔야 합니다. JavaScript의 `Map`은 배열도 키로 받지만 **참조**로 비교하므로 내용이 같은 다른 배열은 다른 키가 됩니다. 내용으로 묶으려면 문자열로 만들어 키를 삼으세요 — 4주차 애너그램 묶기에서 이 기법을 씁니다.",
    },
    {
      kind: "heading",
      body: "빈도수 세기가 절반입니다",

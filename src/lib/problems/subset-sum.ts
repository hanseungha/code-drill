...
export const subsetSum: Problem = {
  ...
  difficulty: "medium",
  tags: ["완전탐색", "비트마스크"],
  summary: "비트마스크로 2ⁿ개의 부분집합을 빠짐없이 훑어요.",
  description: [
    "정수 배열 `nums`와 정수 `target`이 주어져요. 원소의 합이 `target`이 되는 **공집합이 아닌 부분집합의 개수**를 반환해요.",
    "값이 같아도 인덱스가 다르면 다른 부분집합이에요. 예를 들어 `nums = [1, 1]` 에서 합이 `1`인 부분집합은 두 개예요.",
    "원소가 `n`개면 부분집합은 `2ⁿ`개예요. `0`부터 `2ⁿ - 1`까지의 정수 하나하나를 **어떤 원소를 골랐는지 나타내는 비트열**로 보면, 반복문 하나로 모든 부분집합을 훑을 수 있어요. `i`번 비트가 켜져 있는지는 `mask & (1 << i)` 로 확인해요.",
  ],
  examples: [
    {
      input: "nums = [1, 2, 3], target = 3",
      output: "2",
      explain: "{3} 과 {1, 2} 두 가지예요.",
    },
    {
      input: "nums = [1, 1, 1], target = 2",
      output: "3",
      explain: "인덱스 기준으로 (0,1), (0,2), (1,2) 세 가지예요.",
    },
...
`,
  ...
  hint: "`for (let mask = 1; mask < (1 << n); mask++)` 로 돌면 공집합인 `0`을 자연스럽게 건너뛰어요. 안쪽에서 `i`번 비트가 켜졌는지 확인하며 합을 만들어요. `1 << 15` 는 32,768 이라 전부 훑어도 순식간이에요.",
  solution: {

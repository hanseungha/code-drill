...
export const rangeAdd: Problem = {
  ...
  difficulty: "medium",
  tags: ["누적합", "배열"],
  summary: "구간 갱신을 양끝 두 칸에만 기록해두고, 마지막에 한 번에 펼칩니다.",
  description: [
    "길이 `n`인 배열이 있고 모든 칸이 `0`으로 시작합니다. 갱신 목록 `updates`가 주어지며, 각 갱신은 `[l, r, v]` 형태로 `l`번부터 `r`번까지(양끝 포함) 모든 칸에 `v`를 더하라는 뜻입니다.",
    "모든 갱신을 끝낸 뒤의 배열을 반환하세요.",
    "갱신마다 구간을 직접 돌면 `O(nm)`입니다. **차분 배열**을 쓰면 갱신 하나가 두 칸 수정으로 끝납니다. `diff[l] += v`, `diff[r + 1] -= v` 를 기록해 두고, 마지막에 `diff`의 누적합을 구하면 그것이 답입니다. 3주차의 누적합을 거꾸로 쓰는 셈입니다.",
  ],
  examples: [
    {
      input: "n = 5, updates = [[0, 2, 1], [1, 3, 2]]",
      output: "[1, 3, 3, 2, 0]",
      explain: "첫 갱신으로 [1,1,1,0,0], 두 번째 갱신으로 1~3번 칸에 2가 더해집니다.",
    },
    { input: "n = 3, updates = []", output: "[0, 0, 0]", explain: "갱신이 없으면 그대로입니다." },
...
`,
  ...
  hint: "차분 배열의 길이를 `n + 1`로 잡으면 `r`이 마지막 칸일 때 `diff[r + 1]` 이 배열 밖으로 나가는 걱정을 하지 않아도 됩니다. 마지막에 앞에서부터 누적해 더하면서 앞 `n`칸만 반환하세요.",
  solution: {

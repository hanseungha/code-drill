...
export const minHeap: Problem = {
  ...
  difficulty: "medium",
  tags: ["힙", "자료구조"],
  summary: "push · pop · peek 연산을 처리하는 최소 힙을 직접 만들어요.",
  description: [
    "연산 목록 `operations`가 주어져요. 최소 힙을 직접 만들어 순서대로 처리하고, 값을 꺼내는 연산의 결과만 모아 배열로 반환해요.",
    "연산은 세 가지예요. `[\"push\", x]`는 x를 넣고 아무것도 반환하지 않아요. `[\"pop\"]`은 가장 작은 값을 꺼내 반환해요. `[\"peek\"]`은 가장 작은 값을 꺼내지 않고 확인만 해요. 힙이 비어 있으면 `pop`과 `peek` 모두 `null`을 반환해요.",
    "결과 배열에는 `pop`과 `peek`의 반환값만 순서대로 담아요.",
    "이 문제의 목적은 정렬 함수를 부르는 게 아니라 **힙 내부 동작을 직접 만들어 보는 거예요**. 배열을 완전 이진 트리로 보고, 인덱스 `i`의 부모가 `(i-1)/2`, 자식이 `2i+1`과 `2i+2`라는 규칙 위에서 위로 올리기(sift up)와 아래로 내리기(sift down)를 구현해 보세요.",
    "Python에는 `heapq`가 내장되어 있지만, 이 문제만큼은 직접 만들어 보길 권해요. 다음 문제부터는 마음껏 써요.",
  ],
  examples: [
    {
      input: 'operations = [["push", 5], ["push", 3], ["push", 8], ["pop"], ["pop"], ["pop"]]',
      output: "[3, 5, 8]",
      explain: "넣은 순서와 무관하게 작은 값부터 나와요.",
    },
    {
      input: 'operations = [["push", 4], ["peek"], ["push", 2], ["peek"], ["pop"], ["peek"]]',
      output: "[4, 2, 2, 4]",
      explain: "peek은 꺼내지 않으니까 두 번째와 세 번째 결과가 모두 2예요.",
    },
    {
      input: 'operations = [["push", 1], ["pop"], ["pop"]]',
      output: "[1, null]",
      explain: "두 번째 pop은 힙이 비어 있어 null이에요.",
    },
  ],
  constraints: [
...
`,
  ...
  hint: "push는 배열 끝에 넣고 부모보다 작은 동안 부모와 맞바꾸며 올라가요. pop은 뿌리를 빼내고 마지막 원소를 뿌리에 올린 뒤, 두 자식 중 더 작은 쪽과 맞바꾸며 내려가요. 두 연산 모두 높이만큼만 움직이니까 O(log n)이에요.",
  solution: {

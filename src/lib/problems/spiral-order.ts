...
export const spiralOrder: Problem = {
  ...
  difficulty: "medium",
  tags: ["시뮬레이션", "배열"],
  summary: "행렬을 바깥에서 안으로 시계 방향으로 감아 읽어요.",
  description: [
    "`n × m` 정수 행렬 `grid`가 주어져요. 왼쪽 위에서 출발해 **시계 방향으로 안쪽으로 감아 들어가며** 읽은 순서대로 1차원 배열에 담아 반환해요.",
    "행과 열의 개수가 다를 수 있고, 한 줄짜리 행렬도 들어와요.",
    "경계를 네 개(위·아래·왼쪽·오른쪽)로 들고 한 줄 읽을 때마다 좁히는 방식이 가장 안전해요. 방향을 바꿀 때마다 **이미 읽은 줄을 다시 읽지 않도록** 조건을 넣는 것이 이 문제의 전부예요.",
  ],
  examples: [
    {
      input: "grid = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]",
      output: "[1, 2, 3, 6, 9, 8, 7, 4, 5]",
      explain: "바깥 테두리를 한 바퀴 돈 뒤 가운데 5가 남아요.",
    },
    { input: "grid = [[1, 2, 3, 4]]", output: "[1, 2, 3, 4]", explain: "한 행뿐이면 그대로예요." },
...
`,
  ...
  hint: "`top`, `bottom`, `left`, `right` 네 변수를 두고 → ↓ ← ↑ 순으로 한 줄씩 읽으며 안쪽으로 좁혀요. ← 와 ↑ 를 읽기 전에 `top <= bottom`, `left <= right` 를 다시 확인해야 한 줄짜리 행렬에서 같은 칸을 두 번 담지 않아요.",
  solution: {

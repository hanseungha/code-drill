...
export const longestConsecutive: Problem = {
  ...
  difficulty: "hard",
  tags: ["해시", "배열"],
  summary: "정렬하지 않고 셋만으로 가장 긴 연속 구간의 길이를 구해요.",
  description: [
    "정수 배열 `nums`가 주어져요. 값이 연속으로 이어지는 부분수열 중 **가장 긴 것의 길이**를 반환해요. 원래 배열에서의 순서는 상관없고, 중복된 값은 한 번만 세요.",
    "정렬하면 `O(n log n)`에 쉽게 풀려요. 여기서는 **셋만으로 `O(n)`**에 푸는 게 목표예요.",
    "핵심은 어디서부터 세기 시작할지 고르는 거예요. 모든 값에서 시작해 세면 같은 구간을 몇 번씩 다시 세지만, `x - 1`이 셋에 없는 값 — 즉 **구간의 시작점에서만** 세면 각 값을 정확히 한 번씩만 방문해요.",
  ],
  examples: [
    {
      input: "nums = [100, 4, 200, 1, 3, 2]",
      output: "4",
      explain: "1, 2, 3, 4 가 이어지니까 길이 4 예요.",
    },
    { input: "nums = [1, 1, 1]", output: "1", explain: "중복은 한 번만 세요." },
...
`,
  ...
  hint: "값을 전부 셋에 넣고, 셋을 돌면서 `x - 1`이 셋에 없는 값에서만 `x + 1, x + 2, ...` 로 세어 나가요. 안쪽 반복문이 있어도 전체는 `O(n)`이에요 — 각 값은 자기가 속한 구간에서 딱 한 번만 세어지거든요.",
  solution: {

...
export const quotientRemainder: Problem = {
  ...
  difficulty: "easy",
  tags: ["수학", "구현"],
  summary: "정수 나눗셈의 몫과 나머지를 두 언어에서 같은 규칙으로 구합니다.",
  description: [
    "정수 `a`와 `b`가 주어집니다. `a`를 `b`로 나눈 몫과 나머지를 `[몫, 나머지]` 배열로 반환하세요.",
    "몫은 **0 방향으로 버립니다**. `-7`을 `2`로 나눈 몫은 `-4`가 아니라 `-3`입니다. 나머지는 `a - b * 몫` 으로 정의하므로 나머지의 부호는 `a`를 따라갑니다.",
    "두 언어의 기본 연산자가 서로 다른 답을 내는 문제입니다. 언어에 맞춰 맞추는 것이 이 문제의 전부입니다.",
  ],
  examples: [
    { input: "a = 7, b = 2", output: "[3, 1]" },
    {
      ...
      output: "[-3, -1]",
      explain:
        "몫을 0 방향으로 버려 -3, 나머지는 -7 - 2 × (-3) = -1 입니다. 파이썬의 `-7 // 2`는 -4, `-7 % 2`는 1이라 그대로 쓰면 틀립니다.",
    },
    { input: "a = 7, b = -2", output: "[-3, 1]" },
  ],
  constraints: [
...
`,
  ...
  hint: "자바스크립트의 `/`는 실수 나눗셈이라 `Math.trunc`가 필요하고, 파이썬의 `//`는 `-∞` 방향으로 버립니다. 파이썬에서는 절댓값으로 몫을 구한 뒤 부호를 따로 붙이면 됩니다. 나머지는 몫만 정하고 나면 `a - b * 몫` 한 줄입니다.",
  solution: {

...
export const carpet: Problem = {
  ...
  difficulty: "medium",
  tags: ["완전탐색", "수학"],
  summary: "테두리와 안쪽 격자 수만 보고 카펫의 가로세로를 되찾습니다.",
  description: [
    "카펫의 바깥 한 줄은 갈색, 안쪽은 노란색으로 칠해져 있습니다. 갈색 격자 수 `brown`과 노란색 격자 수 `yellow`가 주어질 때, 카펫의 `[가로, 세로]` 를 반환하세요.",
    "가로는 세로보다 크거나 같습니다. 답은 항상 하나뿐입니다.",
    "전체 격자 수는 `brown + yellow` 이고, 이는 `가로 × 세로` 입니다. 즉 **전체 격자 수의 약수만 후보**입니다. 후보가 몇 개 안 되므로 하나씩 넣어 확인하면 됩니다 — 1주차의 약수 구하기와 4주차의 완전탐색이 만나는 지점입니다.",
  ],
  examples: [
    {
      input: "brown = 10, yellow = 2",
      output: "[4, 3]",
      explain: "4 × 3 = 12 개 중 안쪽 2 × 1 = 2 개가 노란색입니다.",
    },
...
`,
  ...
  hint: "전체 격자 수 `total = brown + yellow` 의 약수 `h` 를 세로로 두고 `w = total / h` 를 가로로 두세요. `w ≥ h` 이고 `(w - 2) * (h - 2) === yellow` 이면 답입니다. 1주차처럼 `h * h ≤ total` 까지만 훑으면 충분합니다.",
  solution: {

...
export const compressString: Problem = {
  ...
  difficulty: "medium",
  tags: ["문자열", "구현"],
  summary: "같은 문자가 이어지는 구간을 '문자 + 개수'로 줄여요.",
  description: [
    "문자열 `s`가 주어져요. 같은 문자가 연속으로 나오는 구간을 `문자 + 개수` 로 바꾼 문자열을 반환해요.",
    "개수가 `1`인 구간은 **개수를 쓰지 않아요**. 예를 들어 `\"abc\"` 는 `\"a1b1c1\"` 이 아니라 `\"abc\"` 그대로예요.",
    "연속 구간을 세는 일 자체는 반복문 하나면 되지만, 마지막 구간을 빠뜨리기 쉬워요. 반복문이 끝난 뒤에 한 번 더 기록하거나, 경계를 넘어설 때 기록하도록 조건을 잡아요.",
  ],
  examples: [
    { input: 's = "aabbbcc"', output: '"a2b3c2"' },
    { input: 's = "abc"', output: '"abc"', explain: "모든 구간의 길이가 1이라 개수를 쓰지 않아요." },
    { input: 's = "aaaaaaaaaaaa"', output: '"a12"', explain: "개수가 두 자리여도 그대로 붙여요." },
  ],
...
`,
  ...
  hint: "조각을 배열에 모았다가 마지막에 한 번만 이어붙여요. 반복문 안에서 `result += ...` 로 문자열을 계속 새로 만들면 길이가 길어질수록 급격히 느려져요. 자바스크립트는 `join(\"\")`, 파이썬은 `\"\".join(...)` 이에요.",
  solution: {

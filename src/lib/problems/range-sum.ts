...
export const rangeSum: Problem = {
  ...
  difficulty: "medium",
  tags: ["누적합", "배열"],
  summary: "질문이 많을 때, 미리 한 번 계산해두고 각 질문에 O(1)로 답합니다.",
  description: [
    "정수 배열 `nums`와 질문 목록 `queries`가 주어집니다. 각 질문은 `[l, r]` 형태이고, `nums[l]` 부터 `nums[r]` 까지(양끝 포함)의 합을 뜻합니다. 질문마다의 답을 순서대로 배열에 담아 반환하세요.",
    "질문이 들어올 때마다 `l`부터 `r`까지 더하면 질문 하나가 `O(n)`이고, 질문이 10만 개면 `O(nq)`가 되어 늦습니다.",
    "**누적합**을 미리 만들어 두면 각 질문은 뺄셈 한 번입니다. `pre[i]` 를 앞에서부터 `i`개의 합이라고 두면, `l..r` 의 합은 `pre[r + 1] - pre[l]` 입니다. 길이를 `n + 1`로 잡는 이유는 `l = 0` 일 때 따로 처리하지 않기 위해서입니다.",
  ],
  examples: [
    {
      input: "nums = [1, 2, 3, 4, 5], queries = [[0, 2], [1, 3], [4, 4]]",
      output: "[6, 9, 5]",
      explain: "1+2+3 = 6, 2+3+4 = 9, 5 = 5 입니다.",
    },
...
`,
  ...
  hint: "누적합 배열을 `pre[0] = 0`, `pre[i + 1] = pre[i] + nums[i]` 로 만드세요. 그러면 질문 하나는 `pre[r + 1] - pre[l]` 로 끝납니다. `r + 1` 을 쓰기 때문에 배열 길이는 `n + 1` 이어야 합니다.",
  solution: {

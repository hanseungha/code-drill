...
export const rotateMatrix: Problem = {
  ...
  difficulty: "medium",
  tags: ["배열", "구현"],
  summary: "n×n 행렬을 시계 방향으로 90도 돌려요.",
  description: [
    "`n × n` 정수 행렬 `matrix`가 주어져요. 시계 방향으로 90도 돌린 행렬을 반환해요.",
    "인덱스를 한 번에 옮기려 하면 헷갈려요. **전치한 뒤 각 행을 뒤집는다**로 나누면 두 단계 모두 실수할 여지가 거의 없어요. 전치는 `(i, j)` 와 `(j, i)` 를 맞바꾸는 것이고, 그 다음 각 행을 뒤집으면 시계 방향 회전이 돼요.",
    "어려운 문제를 아는 조각으로 쪼개는 연습이에요. 반시계 방향이 필요하면 순서를 바꿔 각 행을 먼저 뒤집고 전치하면 돼요.",
  ],
  examples: [
    {
      input: "matrix = [[1, 2], [3, 4]]",
      output: "[[3, 1], [4, 2]]",
      explain: "왼쪽 아래의 3이 왼쪽 위로 올라와요.",
    },
    {
      input: "matrix = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]",
      output: "[[7, 4, 1], [8, 5, 2], [9, 6, 3]]",
      explain: "첫 열을 아래에서 위로 읽은 것이 새 첫 행이에요.",
    },
    { input: "matrix = [[1]]", output: "[[1]]" },
  ],
  constraints: ["1 ≤ n ≤ 200", "-10^6 ≤ matrix[i][j] ≤ 10^6", "행과 열의 개수는 같아요."],
...
`,
  ...
  hint: "새 행렬을 하나 만들어 `out[j][n - 1 - i] = matrix[i][j]` 로 옮겨도 돼요. 제자리에서 하려면 전치 후 각 행 뒤집기가 가장 헷갈리지 않아요. 전치할 때는 `j`를 `i + 1`부터 돌아야 두 번 바꿔 제자리로 돌아오지 않아요.",
  solution: {

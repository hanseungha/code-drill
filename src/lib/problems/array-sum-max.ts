...
export const arraySumMax: Problem = {
  ...
  difficulty: "easy",
  tags: ["배열", "구현"],
  summary: "배열을 한 번 훑어 합과 최댓값을 함께 구합니다.",
  description: [
    "정수 배열 `nums`가 주어집니다. 모든 원소의 합과 가장 큰 값을 `[합, 최댓값]` 배열로 반환하세요.",
    "언어가 제공하는 배열 API를 써도 되고 직접 반복문을 돌려도 됩니다. 다만 **원소가 10만 개까지 들어온다**는 점은 기억해 두세요. 편해 보이는 방법 중에 이 크기에서 터지는 것이 있습니다.",
  ],
  examples: [
...
`,
  ...
  hint: "자바스크립트의 `Math.max(...nums)` 는 배열을 인자 목록으로 펼치기 때문에 원소가 몇만 개가 되면 스택이 터집니다. `reduce` 나 평범한 반복문을 쓰세요. 파이썬의 `sum`, `max` 는 이런 제한이 없습니다.",
  solution: {

...
- `/week/[n]` — 개념 노트, 패턴 코드, 그 주의 문제, 프로그래머스 추천 문제, 셀프 체크
- `/` — 문제 전체 목록 (커리큘럼 순서)
...
주차마다 그 주제로 골라둔 프로그래머스 문제 목록이 접힌 채로 붙어 있습니다.
**코딩테스트 고득점 Kit**을 뼈대로, 초심자용 Lv.1 문제와 유명 카카오 기출을
얹어 **67문제**를 주제별로 정리했습니다.

담는 것은 **목록뿐입니다** — 문제 번호, 제목, Lv.1~Lv.5 난이도. 지문과 채점
데이터는 프로그래머스 것이라 이 사이트는 링크만 겁니다. 채점은 프로그래머스에서
하고 체크박스로 표시해 두는 방식입니다.

프로그래머스는 백준처럼 공개 인덱스를 내려받을 곳이 없어, 목록은
`src/lib/programmers/recommended.json`에 손으로 관리합니다. 이 파일은 커밋되어
있어서 빌드나 요청 때 네트워크를 타지 않습니다.
...
src/lib/programmers/  프로그래머스 추천 문제 목록 (손으로 관리)
